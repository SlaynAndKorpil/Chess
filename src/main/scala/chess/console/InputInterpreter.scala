...
class InputInterpreter extends ChessIO with CommandRegistrator {
  ...
  /**
    * Runs an interpretation of the console and plays the interpreted moves on the [[chess.console.InputInterpreter#board board]].
    */
  def run(): Unit = {
    println(board)
    gameLoop()
  ...
  chessReactions += {
    case ShowEnded(result) =>
      import DrawResultReason._
      import WinResultReason._
